...
bool load_opl3() {
    ...
    // Load PIO program
    used_offset = pio_manager_load(&used_pio, &used_sm, &opl3_program);
    ...
    pio_sm_config used_config = opl3_program_get_default_config(used_offset);
#if LPT_STROBE_SWAPPED
...
bool unload_opl3() {
    ...
    pio_manager_unload(used_pio, used_sm, used_offset, &opl3_program);
...
size_t generate_opl3(int16_t *left_sample, int16_t *right_sample) {
...
device_t *create_opl3(void) {
    static device_t opl3_struct = {load_opl3, unload_opl3, generate_opl3};
    return &opl3_struct;
}