...
#ifdef DBOPL_USE_PRECALCUALTED_TABLES
#include "tables.ipp"
#else
...
#endif
#else 
	// ignore scale and force 49716 Hz rendering, just like real OPL3 would
	int scale = 1;

	//Noise counter is run at the same precision as general waves
	noiseAdd = (uint32_t)(scale * (1 << LFO_SH));
	noiseCounter = 0;
	noiseValue = 1;	//Make sure it triggers the noise xor the first time
	//The low frequency oscillation counter
	//Every time his overflows vibrato and tremoloindex are increased
	lfoAdd = (uint32_t)(scale * (1 << LFO_SH));
	lfoCounter = 0;
	vibratoIndex = 0;
	tremoloIndex = 0;
...
#else
	uint32_t freqScale = (uint32_t)(scale * (1 << (WAVE_SH - 1 - 10)));
	for (int i = 0; i < 16; i++) {
		freqMul[i] = freqScale * FreqCreateTable[i];
	}
#endif
...
void InitTables( void ) {
...
#ifndef DBOPL_USE_PRECALCUALTED_TABLES
#if ( DBOPL_WAVE == WAVE_HANDLER ) || ( DBOPL_WAVE == WAVE_TABLELOG )
	...
	// dump tables to be reused later
#ifdef DBOPL_DUMP_TABLES
	{
		FILE* f = fopen("tables.ipp", "w");
		if (f) {
			fprintf(f, "// autogenerated tables\n\n");

#if ( DBOPL_WAVE == WAVE_HANDLER ) || ( DBOPL_WAVE == WAVE_TABLELOG )
			fprintf(f, "static uint16_t ExpTable[ 256 ] = {\n");
			for (int i = 0; i < 256; i++) {
				if ((i & 15) == 0) fprintf(f, "    ");
				fprintf(f, "%6u, ", ExpTable[i]);
...
#if ( DBOPL_WAVE == WAVE_TABLEMUL )
			fprintf(f, "static uint16_t MulTable[ 384 ] = {\n");
			for (int i = 0; i < 384; i++) {
				if ((i & 15) == 0) fprintf(f, "    ");
				fprintf(f, "%6u, ", MulTable[i]);
				if ((i & 15) == 15) fprintf(f, "\n");
			}
			fprintf(f, "};\n\n");
